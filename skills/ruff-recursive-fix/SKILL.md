...
- Inline `# noqa` suppression as a last resort (see Suppression Decision below).
...
2. Else if `uv` is on PATH and a `pyproject.toml` with `[tool.ruff]` exists, use `uv run ruff`.
3. Else if `ruff` is on PATH, use `ruff`.
4. Else if `python -m ruff --version` succeeds, use `python -m ruff`.
5. Else stop and ask the user how to invoke Ruff in this project.
