...
- Prefer lower-risk deployment windows; flag Friday/end-of-week deploys as elevated risk
- Verify each step before proceeding to the next
- Validate assumptions with evidence, not intuition

## Proportional Effort

Scale the rollout plan to match the change's blast radius:

- **Small** (config change, single service, non-customer-facing): abbreviated plan
  with rollback and one verification step.
- **Medium** (multi-service, schema migration, customer-facing): full plan with
  canary/blue-green, verification signals, and communication.
- **Large** (cross-region, data migration, breaking change): full plan plus
  staged rollout, explicit go/no-go gates, and extended monitoring.
